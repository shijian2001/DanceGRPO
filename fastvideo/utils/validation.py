    ...
    for validation_sampling_step in args.validation_sampling_steps.split(","):
        ...
        for validation_guidance_scale in validation_guidance_scale_ls:
            ...
            local_prompt_ids = validation_prompt_ids[
                nccl_info.group_id * num_embeds_per_group : (nccl_info.group_id + 1) * num_embeds_per_group
            ]
            ...
            for i in local_prompt_ids:
                ...
                prompt_embeds = (
                    torch.load(prompt_embed_path, map_location="cpu", weights_only=True).to(device).unsqueeze(0)
                )
                prompt_attention_mask = (
                    torch.load(prompt_mask_path, map_location="cpu", weights_only=True).to(device).unsqueeze(0)
                )
